...
def nginx_update_config
  ...
  proxy_path   = "\n\n\t\tproxy_path #{ENV.fetch('PROXY_PATH', nil)};\n\n"

  output_file = File.open('tmp/ptool-subdomains.conf', 'w') do |file|
  ...
  `mv tmp/#{nginx_config_name} /etc/nginx/site-available/#{nginx_config_name}`
  nginx_restart
end
